...
class TaxDataManager:
    def __init__(self, bracket_file=None):
        ...
        if bracket_file:
            ...
            self.bracket_file = os.path.join(self.base_dir, 'resources', 'tax_foundation_tax_rates.xlsx')

        self.raw_bracket_df = self.load_bracket_data()

    def load_bracket_data(self):
        if os.path.exists(self.bracket_file):
            return pd.read_excel(self.bracket_file, skiprows=1)
        return pd.DataFrame()
    ...
    def get_personal_exemption(self, year, status='single', dependents=0):
        ...
        try:
            ...
            # Use your specific keys: single, married, dependent
    ...
    def get_clean_income_tax_data(self, year, status='single'):
        # Matches your mapping: Joint=1, Separate=4, Single=7, HoH=10
        filing_status_map = {
            "single": 7,
            "married_joint": 1,
            "married_separate": 4,
